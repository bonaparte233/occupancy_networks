...
#  Further modified for modern Python compatibility
...
def read_as_3d_array(fp, fix_coords=True):
    ...
    data = np.repeat(values, counts).astype(bool)  # Updated for modern numpy
...
def read_as_coord_array(fp, fix_coords=True):
    ...
    values = values.astype(bool)  # Updated for modern numpy
    ...
    x = nz_voxels // (dims[0]*dims[1])  # Use integer division
    zwpy = nz_voxels % (dims[0]*dims[1])  # z*w + y
    z = zwpy // dims[0]  # Use integer division
    y = zwpy % dims[0]
    if fix_coords:
...
def dense_to_sparse(voxel_data, dtype=int):  # Updated default dtype
...
def sparse_to_dense(voxel_data, dims, dtype=bool):  # Updated default dtype
...
def write(voxel_model, fp):
    ...
    for c in voxels_flat:
        if c == state:
            ...
            if ctr == 255:
                fp.write(bytes([int(state)]))  # Convert to bytes properly
                fp.write(bytes([ctr]))
                ctr = 0
        else:
