...
class OccupancyNetwork(nn.Module):
    ...
    def forward(self, p, inputs=None, sample=True, **kwargs):
        """Perform a forward pass through the network.

        Args:
            p (tensor): points
            inputs (tensor): conditioning inputs
    ...
    def compute_elbo(self, p, occ, inputs, **kwargs):
        ...
        rec_error = -p_r.log_prob(occ).sum(-1)
        kl = dist.kl_divergence(q_z, self.p0_z).sum(-1)
        elbo = -rec_error - kl
    ...
    def encode_inputs(self, inputs):
        ...
        if self.encoder is not None:
            ...
            c = torch.zeros(batch_size, 0).to(self._device)
    ...
    def get_z_from_prior(self, size=torch.Size([]), sample=True):
        """Return z from prior distribution.
        ...
        if sample:
            ...
            z = self.p0_z.mean
            z = z.expand(*size, *z.size()).to(self._device)
